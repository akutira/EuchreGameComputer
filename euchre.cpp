...
class Game{
    ...
    public:
        ...
        Game(vector<Player*> playmakers, Pack cards, bool shuffled, 
        int finish_line_points) {
            players = playmakers;
        ...
        void deal_round(Player* player, int round) {
        ...
        void deal(Player* dealer) {
            ...
            while (round < 3) {
                deal_round(player, round);
                round++;
            }
        }

        bool try_order_up(const Card& upcard, Player* player, Suit upsuit, int round) {
            Player* dealer = players[0];
            bool ordered_up = player->make_trump(upcard, player == dealer, round, 
            upsuit);
            if (ordered_up) { 
                trump = upsuit;
        ...
        bool make_trumpGame(const Card& upcard, Player* dealer, Suit upsuit, int round){
            ...
            for (int i = 0; i < 4; i++) {  
                bool orderedup = try_order_up(upcard, trump_maker, upsuit, round);
                if (orderedup) { 
                    return true;
                }
